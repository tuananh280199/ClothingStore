...
const TopProduct = (props) => {
  ...
  useEffect(() => {
    dispatch(actionCreator.fetchTopProducts());
  }, []);

  const goToDetailProduct = (detailProduct) => {
      ...
      <FlatList
        ...
        renderItem={({item}) => (
          <TouchableOpacity
            ...
            activeOpacity={0.2}
            onPress={() => goToDetailProduct(item)}>
            <Image
  ...
  return (
    <View style={container}>
      ...
      {loading ? <Loading /> : viewTopProduct}
    </View>
  );
};
